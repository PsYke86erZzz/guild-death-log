...
local TITLE_DEFS = {
    ...
    {id = "perfectionist", name = locale == "deDE" and "der Perfektionist" or "the Perfectionist", nameFemale = locale == "deDE" and "die Perfektionistin" or "the Perfectionist", desc = locale == "deDE" and "Level 60 + Alle Berufe 300 + Alle Dungeons" or "Level 60 + All Profs 300 + All Dungeons", icon = "Interface\\Icons\\Spell_Holy_PrayerOfFortitude", color = {0.9, 0.8, 0.5}, requirement = "meta_perfectionist", priority = 480},
    ...
    -- ===========================================================
    -- ⚔️ HARDCORE DUELL-TITEL - DIE ULTIMATIVEN! ⚔️
    -- ===========================================================
    {id = "hand_of_fate", name = locale == "deDE" and "Hand des Schicksals" or "Hand of Fate", nameFemale = locale == "deDE" and "Hand des Schicksals" or "Hand of Fate", desc = locale == "deDE" and "25 Duelle auf Leben und Tod ueberlebt!" or "Survived 25 Duels to the Death!", icon = "Interface\\Icons\\Spell_Shadow_Requiem", color = {0.9, 0.2, 0.9}, requirement = "duel_survivor_25", priority = 550},
    {id = "immortal_gladiator", name = locale == "deDE" and "der Unsterbliche Gladiator" or "the Immortal Gladiator", nameFemale = locale == "deDE" and "die Unsterbliche Gladiatorin" or "the Immortal Gladiator", desc = locale == "deDE" and "10 Duelle auf Leben und Tod ueberlebt" or "Survived 10 Duels to the Death", icon = "Interface\\Icons\\Spell_Shadow_DeathPact", color = {0.8, 0.3, 0.3}, requirement = "duel_survivor_10", priority = 520},
    {id = "blood_judge", name = locale == "deDE" and "Blutrichter" or "Blood Judge", nameFemale = locale == "deDE" and "Blutrichterin" or "Blood Judge", desc = locale == "deDE" and "3 Duelle auf Leben und Tod ueberlebt" or "Survived 3 Duels to the Death", icon = "Interface\\Icons\\Ability_Warrior_BloodFrenzy", color = {0.7, 0.2, 0.2}, requirement = "duel_survivor_3", priority = 490},
    {id = "unforgotten", name = locale == "deDE" and "der Unvergessliche" or "the Unforgotten", nameFemale = locale == "deDE" and "die Unvergessliche" or "the Unforgotten", desc = locale == "deDE" and "Ein Duell auf Leben und Tod ueberlebt!" or "Survived a Duel to the Death!", icon = "Interface\\Icons\\Ability_DualWieldSpecialization", color = {0.6, 0.3, 0.3}, requirement = "duel_survivor_1", priority = 460},
}
...
local TITLE_LOGIN_DELAY = 8  -- Sekunden warten nach Login

-- Character-spezifischer Key (wie bei Milestones)
function Titles:GetCharacterKey(name, realm)
    if not name then
...
-- Character-spezifische Titel-Daten holen
function Titles:GetCharacterData(charKey)
    charKey = charKey or self:GetCharacterKey()
    GuildDeathLogDB.titles[charKey] = GuildDeathLogDB.titles[charKey] or {
        selected = nil
    }
    return GuildDeathLogDB.titles[charKey]
end
...
function Titles:Initialize()
    ...
    -- Datenbank initialisieren (Character-spezifisch!)
    GuildDeathLogDB.titles = GuildDeathLogDB.titles or {}
    
    -- MIGRATION: Alte globale "selected" in character-spezifisch konvertieren
    if GuildDeathLogDB.titles.selected and type(GuildDeathLogDB.titles.selected) ~= "table" then
        local oldSelected = GuildDeathLogDB.titles.selected
        GuildDeathLogDB.titles.selected = nil -- Alte Struktur löschen
        local charKey = self:GetCharacterKey()
        local charData = self:GetCharacterData(charKey)
        charData.selected = oldSelected
        GDL:Debug("Titles: Migration von globaler zu character-spezifischer Speicherung")
    end
    ...
    -- Mit Login-Delay starten (Server ist nach Login "cranky")
    C_Timer.After(TITLE_LOGIN_DELAY, function() 
        self:BroadcastTitle() 
    end)
    C_Timer.NewTicker(BROADCAST_INTERVAL, function() self:BroadcastTitle() end)
    
    -- Bei Login: Titel von anderen anfragen (nach Delay)
    C_Timer.After(TITLE_LOGIN_DELAY + 2, function() self:RequestAllTitles() end)
    
    GDL:Debug("Titles: Modul initialisiert mit Chat + Tooltip + Nameplate Hooks (Character-spezifisch!)")
end
...
function Titles:GetSelectedTitle()
    ...
    local selectedId = charData and charData.selected
