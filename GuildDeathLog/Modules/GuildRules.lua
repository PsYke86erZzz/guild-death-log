...
function GuildRules:Initialize()
    ...
    -- Mit Login-Delay (Server ist nach Login "cranky")
    C_Timer.After(8, function()
        if IsInGuild() then
            self:RequestRules()
            GDL:Debug("[GuildRules] Automatischer Request gesendet (nach Login-Delay)")
        end
    end)
...
-- Escape-Funktion für problematische Zeichen beim Sync
local function EscapeForSync(text)
    if not text then return "" end
    -- Reihenfolge wichtig: Erst Backslash, dann Pipe
    text = text:gsub("\\", "\\\\")  -- \ → \\
    text = text:gsub("|", "\\P")    -- | → \P
    text = text:gsub("\n", "\\N")   -- Newline → \N
    return text
...
-- Unescape-Funktion beim Empfangen
local function UnescapeFromSync(text)
    if not text then return "" end
...
function GuildRules:OnAddonMessage(message, sender)
    ...
    if cmd == "RULES_UPDATE" then
        ...
        if version > (GuildDeathLogDB.guildRulesVersion or 0) then
            ...
            if guildName then
                -- WICHTIG: Unescape für Sonderzeichen!
                GuildDeathLogDB.guildRules[guildName] = UnescapeFromSync(rulesText)
    ...
    elseif cmd == "RULES_CHUNK" then
        ...
        if version > (GuildDeathLogDB.guildRulesVersion or 0) then
            ...
            if complete then
                ...
                if guildName then
                    -- WICHTIG: Unescape für Sonderzeichen!
                    GuildDeathLogDB.guildRules[guildName] = UnescapeFromSync(fullText)
                    GuildDeathLogDB.guildRulesVersion = version
...
function GuildRules:BroadcastRules(showChatMessage)
    ...
    -- WICHTIG: Escape für Sonderzeichen VOR dem Senden!
    local escapedText = EscapeForSync(rulesText)
    
    -- Max 200 Zeichen pro Chunk (Reserve für Header)
    local CHUNK_SIZE = 180  -- Etwas kleiner wegen Escape-Overhead
    
    if #escapedText <= CHUNK_SIZE then
        -- Kurze Regeln: Direkt senden
        local message = "RULES_UPDATE " .. version .. "|" .. escapedText
        C_ChatInfo.SendAddonMessage("GDLRules", message, "GUILD")
    else
        -- Lange Regeln: In Chunks aufteilen
        local totalChunks = math.ceil(#escapedText / CHUNK_SIZE)
        
        for i = 1, totalChunks do
            local startPos = (i - 1) * CHUNK_SIZE + 1
            local endPos = math.min(i * CHUNK_SIZE, #escapedText)
            local chunk = escapedText:sub(startPos, endPos)
...
function GuildRules:CreateRulesFrame()
    ...
    -- Buttons (nur Speichern und Sync, zentriert)
    local btnWidth = 120
    local btnSpacing = 20
    
    -- Speichern-Button (lokalisiert)
    f.saveBtn = self:CreateRulesButton(f, GDL:L("SAVE"), -(btnWidth/2 + btnSpacing/2), 20, function()
...
-- Auto-Initialize (braucht Sync-Events)
local initFrame = CreateFrame("Frame")
initFrame:RegisterEvent("PLAYER_ENTERING_WORLD")
initFrame:SetScript("OnEvent", function(self, event)
    if event == "PLAYER_ENTERING_WORLD" then
        C_Timer.After(2, function()
            GuildRules:Initialize()
        end)
