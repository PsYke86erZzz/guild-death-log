...
if locale == "deDE" then
    L = {
        ...
        META_ALL_SECONDARY_DESC = "Alle 3 Nebenberufe auf 300 (Angeln, Kochen, Erste Hilfe)",
        META_COMPLETE_CRAFTER = "Der Perfekte Handwerker",
        META_COMPLETE_CRAFTER_DESC = "Ein Hauptberuf auf 300 + alle Nebenberufe",
        ...
        META_PERFECT_DESC = "Level 60 + Meisterhandwerker + Alle Dungeons",
        
        -- HARDCORE DUELL - DER ULTIMATIVE TITEL
        DUEL_SURVIVOR = "Der Unvergessliche",
        DUEL_SURVIVOR_DESC = "Ein Duell auf Leben und Tod UEBERLEBT!",
        DUEL_CHAMPION = "Blutrichter",
        DUEL_CHAMPION_DESC = "3 Duelle auf Leben und Tod ueberlebt",
        DUEL_LEGEND = "Der Unsterbliche Gladiator",
        DUEL_LEGEND_DESC = "10 Duelle auf Leben und Tod ueberlebt!",
        DUEL_GOD = "Hand des Schicksals",
        DUEL_GOD_DESC = "25 Duelle auf Leben und Tod ueberlebt - LEGENDE!",
    ...
    L = {
        ...
        META_ALL_SECONDARY_DESC = "All 3 secondary professions at 300 (Fishing, Cooking, First Aid)",
        META_COMPLETE_CRAFTER = "The Perfect Crafter",
        META_COMPLETE_CRAFTER_DESC = "One main profession at 300 + all secondaries",
        ...
        META_PERFECT_DESC = "Level 60 + Master Crafter + All Dungeons",
        
        -- HARDCORE DUEL - THE ULTIMATE TITLE
        DUEL_SURVIVOR = "The Unforgotten",
        DUEL_SURVIVOR_DESC = "SURVIVED a Duel to the Death!",
        DUEL_CHAMPION = "Blood Judge",
        DUEL_CHAMPION_DESC = "Survived 3 Duels to the Death",
        DUEL_LEGEND = "The Immortal Gladiator",
        DUEL_LEGEND_DESC = "Survived 10 Duels to the Death!",
        DUEL_GOD = "Hand of Fate",
        DUEL_GOD_DESC = "Survived 25 Duels to the Death - LEGEND!",
...
local MILESTONE_DEFS = {
    ...
    {id = "played_720h", name = L.PLAYED_720H, desc = L.PLAYED_720H_DESC, icon = "Interface\\Icons\\Spell_Holy_Resurrection", type = "playtime", threshold = 2592000, category = "survival"},
    ...
    {id = "boss_mc_rag", name = L.BOSS_MC_RAG, desc = L.BOSS_MC_RAG_DESC, icon = "Interface\\Icons\\Spell_Fire_Lavaspawn", type = "boss", bossId = 11502, category = "raid"},
    ...
    {id = "meta_complete_crafter", name = L.META_COMPLETE_CRAFTER, desc = L.META_COMPLETE_CRAFTER_DESC, icon = "Interface\\Icons\\INV_Misc_Food_67", type = "meta", requirements = {"prof_300", "meta_all_secondary"}, category = "meta"},
    ...
    {id = "meta_perfectionist", name = L.META_PERFECT, desc = L.META_PERFECT_DESC, icon = "Interface\\Icons\\Spell_Holy_PrayerOfFortitude", type = "meta", requirements = {"level_60", "meta_complete_crafter", "meta_all_dungeons"}, category = "secret", secret = true},
    
    -- ⚔️ HARDCORE DUELLE - DIE ULTIMATIVEN TITEL ⚔️
    {id = "duel_survivor_1", name = L.DUEL_SURVIVOR, desc = L.DUEL_SURVIVOR_DESC, icon = "Interface\\Icons\\Ability_DualWieldSpecialization", type = "duel", threshold = 1, category = "secret", secret = true},
    {id = "duel_survivor_3", name = L.DUEL_CHAMPION, desc = L.DUEL_CHAMPION_DESC, icon = "Interface\\Icons\\Ability_Warrior_BloodFrenzy", type = "duel", threshold = 3, category = "secret", secret = true},
    {id = "duel_survivor_10", name = L.DUEL_LEGEND, desc = L.DUEL_LEGEND_DESC, icon = "Interface\\Icons\\Spell_Shadow_DeathPact", type = "duel", threshold = 10, category = "secret", secret = true},
    {id = "duel_survivor_25", name = L.DUEL_GOD, desc = L.DUEL_GOD_DESC, icon = "Interface\\Icons\\Spell_Shadow_Requiem", type = "duel", threshold = 25, category = "secret", secret = true},
}
...
function Milestones:RegisterEvents()
    ...
    -- ⚔️ HARDCORE DUELL EVENTS ⚔️
    self.eventFrame:RegisterEvent("DUEL_REQUESTED")
    self.eventFrame:RegisterEvent("DUEL_FINISHED")
    self.eventFrame:RegisterEvent("DUEL_INBOUNDS")
    self.eventFrame:RegisterEvent("DUEL_OUTOFBOUNDS")
    
    -- Duell-Tracking Variablen
    self.inDuel = false
    self.duelOpponent = nil
    self.duelStartTime = nil
end

-- ⚔️ HARDCORE DUELL TRACKING ⚔️
function Milestones:DUEL_REQUESTED(opponentName)
    self.pendingDuel = opponentName
    GDL:Debug("Duel requested by: " .. (opponentName or "unknown"))
end

function Milestones:DUEL_INBOUNDS()
    if self.pendingDuel then
        self.inDuel = true
        self.duelOpponent = self.pendingDuel
        self.duelStartTime = time()
        GDL:Debug("Duel started against: " .. (self.duelOpponent or "unknown"))
    end
end

function Milestones:DUEL_OUTOFBOUNDS()
    -- Spieler ist aus dem Duellbereich - noch im Duell aber Warnung
    GDL:Debug("Duel: Out of bounds warning")
end

function Milestones:DUEL_FINISHED()
    -- Duell beendet - prüfen ob wir gewonnen haben
    if self.inDuel and self.duelOpponent then
        -- Kurze Verzögerung um zu prüfen ob Gegner tot ist
        local opponent = self.duelOpponent
        C_Timer.After(0.5, function()
            self:CheckDuelVictory(opponent)
        end)
    end
    
    self.inDuel = false
    self.duelOpponent = nil
    self.pendingDuel = nil
    self.duelStartTime = nil
end

function Milestones:CheckDuelVictory(opponentName)
    -- Prüfe ob wir noch leben (= wir haben gewonnen)
    if not UnitIsDeadOrGhost("player") then
        ...
        GDL:Print(isDE and 
            ("|cffFF4500⚔️ DUELL GEWONNEN!|r Du hast " .. wins .. " Duelle ueberlebt!") or
            ("|cffFF4500⚔️ DUEL WON!|r You survived " .. wins .. " duels!"))
        
        -- Prüfe Duell-Meilensteine
        self:CheckDuelMilestones(wins)
        
        -- Optional: Gildenchat Nachricht für epische Duelle
        if IsInGuild() and wins >= 1 then
            local msg = isDE and 
                ("[GDL] ⚔️ " .. UnitName("player") .. " hat ein Duell gegen " .. opponentName .. " UEBERLEBT! (" .. wins .. " Siege)") or
                ("[GDL] ⚔️ " .. UnitName("player") .. " SURVIVED a duel against " .. opponentName .. "! (" .. wins .. " wins)")
            -- Nur bei besonderen Meilensteinen (1, 3, 10, 25)
            if wins == 1 or wins == 3 or wins == 10 or wins == 25 or wins == 50 or wins == 100 then
...
function Milestones:CheckDuelMilestones(wins)
    local charKey = self:GetCharacterKey()
    local charName = UnitName("player")
    local charData = self:GetCharacterData(charKey)
    
    for _, m in ipairs(MILESTONE_DEFS) do
        if m.type == "duel" and m.threshold and wins >= m.threshold then
            if not charData.unlocked[m.id] then
                self:UnlockMilestone(m.id, charKey, charName)
            end
        end
    end
end
...
function Milestones:UnlockMilestone(milestoneId, charKey, charName, charLevel, silent)
    ...
    -- Position erfassen für Kartenmarker
    local mapId, posX, posY = nil, 0, 0
...
-- NEU: Für MapMarkers
function Milestones:GetMilestoneById(milestoneId)
    return MILESTONE_BY_ID[milestoneId]
end
