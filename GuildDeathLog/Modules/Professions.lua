...
function Professions:Initialize()
    ...
    self.eventFrame:RegisterEvent("INSPECT_READY")             -- Inspect fertig - Talente sammeln!
    self.eventFrame:RegisterEvent("GROUP_ROSTER_UPDATE")       -- NEU: Gruppe geändert - scannen!
    self.eventFrame:RegisterEvent("RAID_ROSTER_UPDATE")        -- NEU: Raid geändert
    ...
    -- NEU: Automatischer Scan alle 2 Minuten für Gildies in der Nähe
    C_Timer.NewTicker(120, function()
        if IsInGuild() then
            self:ScanNearbyGuildies()
        end
    end)
    
    -- NEU: Initialer Scan nach 15 Sekunden
    ...
    GDL:Debug("Professions-Modul aktiv (mit Auto-Scan)")
end
...
function Professions:OnEvent(event, ...)
    ...
    elseif event == "INSPECT_READY" then
        -- Wenn Inspect fertig, Talente von Gildenmitglied sammeln
        local guid = ...
        self:OnInspectReady(guid)
    elseif event == "GROUP_ROSTER_UPDATE" or event == "RAID_ROSTER_UPDATE" then
        -- NEU: Wenn Gruppe sich ändert, Gildenmitglieder in Gruppe scannen
        C_Timer.After(2, function()
...
-- Spec vom inspizierten Spieler ermitteln
function Professions:GetInspectSpec(unit)
    if not GetNumTalentTabs then return "" end
    
    local maxPoints = 0
    local specName = ""
    
    for i = 1, GetNumTalentTabs(true) do  -- true = inspect
...
-- ══════════════════════════════════════════════════════════════
-- AUTOMATISCHES SCANNEN VON GILDENMITGLIEDERN
-- ══════════════════════════════════════════════════════════════

-- Queue für Inspect-Anfragen (Server-Throttling umgehen)
local inspectQueue = {}
local isInspecting = false
local lastInspectTime = 0
local INSPECT_THROTTLE = 1.5  -- Sekunden zwischen Inspects

-- Scannt alle Gildenmitglieder in der Nähe (Stadt, etc.)
function Professions:ScanNearbyGuildies()
    if not IsInGuild() then return end
    
    local found = 0
    local Guild = GDL:GetModule("Guild")
    
    -- Durch alle Nameplates iterieren
    for i = 1, 40 do
        local unit = "nameplate" .. i
        if UnitExists(unit) and UnitIsPlayer(unit) and not UnitIsUnit(unit, "player") then
            local name = UnitName(unit)
            if name and Guild and Guild:IsMember(name) then
                -- Prüfe ob wir Daten brauchen
                if self:NeedsData(name) then
                    self:QueueInspect(unit, name)
                    found = found + 1
                end
            end
        end
    ...
    -- Auch Target und Mouseover prüfen
    ...
    if found > 0 then
        GDL:Debug("Professions: " .. found .. " Gildenmitglieder in der Naehe gefunden")
        self:ProcessInspectQueue()
    end
end

-- Scannt alle Gruppenmitglieder die auch in der Gilde sind
function Professions:ScanGroupMembers()
    if not IsInGuild() then return end
    ...
    if found > 0 then
        GDL:Debug("Professions: " .. found .. " Gildenmitglieder in Gruppe gefunden")
        self:ProcessInspectQueue()
    end
end

-- Prüft ob wir Daten für einen Spieler brauchen
function Professions:NeedsData(name)
    local data = guildProfessions[name]
    if not data then return true end
    
    -- Brauchen wir Spec?
    if not data.spec or data.spec == "" then return true end
    
    -- Daten älter als 1 Stunde?
    if data.timestamp and (time() - data.timestamp) > 3600 then return true end
    
    return false
end

-- Fügt einen Spieler zur Inspect-Queue hinzu
function Professions:QueueInspect(unit, name)
    -- Nicht doppelt hinzufügen
    for _, entry in ipairs(inspectQueue) do
        if entry.name == name then return end
    end
    
    table.insert(inspectQueue, {unit = unit, name = name})
end

-- Verarbeitet die Inspect-Queue
function Professions:ProcessInspectQueue()
    if isInspecting then return end
    if #inspectQueue == 0 then return end
    
    -- Throttle prüfen
    local now = GetTime()
    if now - lastInspectTime < INSPECT_THROTTLE then
        C_Timer.After(INSPECT_THROTTLE, function()
            self:ProcessInspectQueue()
        end)
        return
    end
    
    -- Nächsten aus Queue holen
    local entry = table.remove(inspectQueue, 1)
    if not entry then return end
    
    -- Prüfen ob Unit noch existiert und in Reichweite
    ...
    isInspecting = true
    lastInspectTime = now
    self.currentInspectUnit = entry.unit
    self.currentInspectName = entry.name
    
    NotifyInspect(entry.unit)
    GDL:Debug("Professions: Inspiziere " .. entry.name)
    
    -- Timeout nach 3 Sekunden
    C_Timer.After(3, function()
        if isInspecting and self.currentInspectName == entry.name then
            isInspecting = false
            self.currentInspectUnit = nil
            self.currentInspectName = nil
            ClearInspectPlayer()
            self:ProcessInspectQueue()
        end
    end)
...
-- Verbesserte OnInspectReady die auch Queue verarbeitet
function Professions:OnInspectReady(guid)
    local unit = self.currentInspectUnit or "target"
    local expectedName = self.currentInspectName
    
    -- Versuche Unit zu finden
    if not UnitExists(unit) or (guid and UnitGUID(unit) ~= guid) then
        unit = "target"
        if not UnitExists(unit) or (guid and UnitGUID(unit) ~= guid) then
            unit = "mouseover"
            if not UnitExists(unit) or (guid and UnitGUID(unit) ~= guid) then
    ...
    -- Prüfe ob Gildenmitglied
    local Guild = GDL:GetModule("Guild")
    local isGuildMember = Guild and Guild:IsMember(name)
    ...
    -- Talente auslesen
    local spec = self:GetInspectSpec(unit)
    local _, classId = UnitClassBase(unit)
    local level = UnitLevel(unit)
    
    -- Daten speichern
    if not guildProfessions[name] then
        guildProfessions[name] = {
            prof1 = {name = "", skill = 0, max = 0},
            prof2 = {name = "", skill = 0, max = 0},
        ...
        if spec and spec ~= "" then
            guildProfessions[name].spec = spec
        end
        guildProfessions[name].level = level or guildProfessions[name].level
        guildProfessions[name].classId = classId or guildProfessions[name].classId
        guildProfessions[name].timestamp = time()
    end
    
    GuildDeathLogDB.professions = guildProfessions
    
    if spec and spec ~= "" then
        GDL:Debug("Professions: " .. name .. " = " .. spec .. " (Lvl " .. (level or "?") .. ")")
    end
    ...
    -- Cleanup und nächster
    isInspecting = false
    self.currentInspectUnit = nil
    self.currentInspectName = nil
    ClearInspectPlayer()
    
    -- Nächsten in Queue verarbeiten
    C_Timer.After(INSPECT_THROTTLE, function()
        self:ProcessInspectQueue()
    end)
end

-- Manueller Scan-Befehl
function Professions:ManualScan()
    GDL:Print("Scanne Gildenmitglieder in der Naehe...")
    inspectQueue = {}  -- Queue leeren
    self:ScanNearbyGuildies()
    self:ScanGroupMembers()
    
    if #inspectQueue == 0 then
        GDL:Print("Keine Gildenmitglieder in Reichweite gefunden.")
    else
        GDL:Print(#inspectQueue .. " Gildenmitglieder werden gescannt...")
    end
end
