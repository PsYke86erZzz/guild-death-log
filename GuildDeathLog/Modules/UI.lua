...
-- Alle Sub-Fenster schliessen (verhindert Überlappung)
function UI:CloseAllSubWindows(exceptFrame)
    local windows = {
        self.settingsFrame,
        self.titlesFrame,
        self.hallOfFameFrame,
        self.statisticsFrame,
        self.achievementsFrame,
    }
    
    -- Module-Fenster
    local moduleWindows = {
        "GuildStats", "Professions", "Memorial", "Calendar", "GuildRules", "GuildLeader", "Export", "Debug", "GuildTracker"
    }
    for _, modName in ipairs(moduleWindows) do
        local mod = GDL:GetModule(modName)
        if mod and mod.frame then
            table.insert(windows, mod.frame)
        end
    end
    
    for _, win in ipairs(windows) do
        if win and win ~= exceptFrame and win:IsShown() then
            win:Hide()
        end
    end
end
...
function UI:CreateChronicle()
    ...
    -- Grabstein-Symbol in der Mitte (Custom Icon)
    local skullDivider = f:CreateTexture(nil, "OVERLAY")
    skullDivider:SetTexture("Interface\\AddOns\\GuildDeathLog\\Textures\\DeathMarker")
    skullDivider:SetSize(20, 20)
    skullDivider:SetPoint("CENTER", divider, "CENTER", 0, 0)
    skullDivider:SetAlpha(0.7)
...
function UI:ShowSettings()
    ...
    self:CloseAllSubWindows(self.settingsFrame)
...
function UI:CreateSettings()
    ...
    for _, opt in ipairs(opts) do
        ...
        cb.key = opt[2]
        -- FIX: Checkbox sofort beim Erstellen initialisieren!
        cb:SetChecked(GuildDeathLogDB.settings[opt[2]] or false)
    ...
    -- FIX: OnShow Handler um Checkboxen IMMER zu aktualisieren
    s.checkboxes = cbs
    s:SetScript("OnShow", function(self)
        for _, cb in ipairs(self.checkboxes) do
            cb:SetChecked(GuildDeathLogDB.settings[cb.key] or false)
        end
    end)
...
function UI:ShowHallOfFame()
    ...
    self:CloseAllSubWindows(self.hallOfFameFrame)
...
function UI:CreateMilestonesWindow()
    ...
    local categories = {
        ...
        {id = "meta", icon = "Interface\\Icons\\INV_Misc_Rune_01", color = {0.6, 0.4, 0.8}},
...
function UI:ToggleTitles()
    ...
    if self.titlesFrame:IsShown() then
        ...
        self:CloseAllSubWindows(self.titlesFrame)
