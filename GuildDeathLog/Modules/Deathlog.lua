...
function Deathlog:TryImportEntry(entry, playerKey, guildData, Sync)
    ...
    -- Name extrahieren (früh, für Blacklist-Check)
    ...
    local timestamp = entry.date or entry.timestamp or entry.time or 0
    
    -- BLACKLIST-CHECK: Wurde dieser Eintrag gelöscht?
    if Sync and Sync:IsDeleted(name, timestamp) then
        GDL:Debug("Deathlog: " .. (name or "?") .. " ist auf Blacklist - ignoriert")
        return false
    end
...
-- Auto-Initialize (scannt nach Deathlog-Addon)
